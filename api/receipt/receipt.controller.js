...
module.exports = {
  generateReceipt: (req, res) => {
    ...
    if (
      !data.trans_id &&
      (!verified ||
        !getObj(verified, "data.position").toLowerCase().includes("admin"))
    )
        ...
        transaction.service_view(
          ...
          (err, results) => {
            ...
            if (!trans_data.length)
              return res.json(
                errorJsonResponse({ detail: "Transaction Not Found" })
              );
            ...
            const doc = new PDFDocument({
              ...
              info: {
                ...
                CreationDate:
                  trans_data.length == 1
                    ? new Date(trans_data[0].date_paid)
                    : today,
                ModDate: today,
              },
            });
            trans_data.forEach((trans, i) => {
              // Content Header
              const trans_receipt_url = `http${req.secure ? "s" : ""}://${
                req.headers.host
              }${req.client.parser.incoming.baseUrl}/${trans.trans_id}`;
              const offset = 20;
              let font_size = 10,
              ...
              if (data.header == "true" || data.header === true)
                doc
                  // column 1
                  .image("./public/favicon-32x32.png", (x += 10), y, {
                    fit: [35, 35],
                    align: "center",
                    valign: "center",
                  })
                  ...
                  .text(
                    String(info.name)
                      .substring(0, info.name_short.length)
                      .toUpperCase(),
                    (x += 40),
                    (y += 7)
                  )
                  ...
                  .text(
                    String(info.name).substring(info.name_short.length + 1),
                    x,
                    (y += font_size + 2)
                  )
              ...
              doc
                ...
                .text(
                  "Account ID",
                  (x = doc.page.margins.left + offset + 20),
                  (y += 30)
                )
                .text(":", x + 200, y)
                ...
                .text(":", x + 200, y)
                .text(`${trans.paymethod}`, x + 230, y)

                // Date Paid
                .text("Date", x, (y += font_size + 10))
                .text(":", x + 200, y)
                .text(
                  `${new Date(trans.date_paid)
                    .toString()
                    .replace(/ GMT.+/, "")}`,
                  x + 230,
                  y
                ...
                .text(":", x + 200, y)
                .text(`${trans.receiver_id}`, x + 230, y);
              ...
              doc
                ...
                .text("Received by", x, (y += font_size + 10))
                .text(":", x + 100, y)
                .text(`${trans.receiver_id}`, x + 110, y)
              ...
              if (data.footer == "true" || data.footer === true)
                doc
                  ...
                  .text(
                    `CAS#: ${trans.trans_id} On: ${new Date(
                      trans.date_paid
                    ).toLocaleDateString()} Series: ${new Date(
                      trans.date_paid
                    ).getFullYear()}`,
                    doc.page.margins.left + 15,
                    500
