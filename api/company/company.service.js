...
module.exports = {
  ...
  service_viewOptions: (data, callBack) => {
    ...
    let pg_query = `SELECT * FROM ${table} WHERE ${__tokey}${data.__match_like ? ' LIKE ' : '='}$1`
    if (data.limit && !isNaN(data.limit)) pg_query = `${pg_query} LIMIT ${data.limit}`
    if (data.offset && !isNaN(data.offset)) pg_query = [pg_query, 'OFFSET', data.offset].join(' ')
    let _doorder = true
    let _noorder = ['LIMIT', 'OFFSET']
    _noorder.forEach((word) => {
      if (pg_query.toLowerCase().includes(` ${word.toLowerCase()} `)) _doorder = false
    })
  ...
  service_updateBySingle: (data, callBack) => {
    let noncol = data.__noncol || ['id', '__noncol', '__important', '__toupdate', '__options'] // keys not to include as columns
    if (noncol.length && !noncol.includes('__noncol')) noncol.push('__noncol')
