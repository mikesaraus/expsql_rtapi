...
module.exports = {
  service_create: (data, callBack) => {
    ...
    let pg_setvals = []
    let pg_query = `INSERT INTO ${table} (trans_id, ${cols}) VALUES(${generateTransactionID(false)},`
  ...
  service_view: (data, callBack) => {
    ...
    let pg_query = `SELECT ${data.count ? 'COUNT(*)' : data.columns ? data.columns : '*'} FROM ${table}`
