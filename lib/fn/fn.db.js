...
module.exports = {
  ...
  queryConditioner: (objData, columnsData) => {
    // columnsData = [ { name: "tbl_column_name", category: "string | number | custom", type: "psql_data_type" } ]; // Array Object Format
    let query_cond = [],
      query_vals = [],
      query_endstement = [];
            ...
            colArray.forEach((v) => {
              if (_or_vals.indexOf(v) === -1) {
                if (
                  ...
                  _or_cond.push([col, column.cond, "NULL"].join(" "));
                } else {
                  _or_cond.push(
                    [
                      col,
    ...
    if (objData.pg_query.toLowerCase().startsWith("select")) {
      ...
      if (objData.groupby) {
        query_endstement.push(
          ["GROUP BY", objData.groupby.split("||").join(", ")].join(" ")
        );
      }
      if (objData.orderby) {
        query_endstement.push(
