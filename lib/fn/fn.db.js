...
module.exports = {
   ...
   * @param {{ code?: string | number, hint?: string, detail?: string, message?: string }| string} errorObj
   * Error from pool query or any custom `object`
   */
  errorJsonResponse: (errorObj) => {
    let response = {
      success: 0,
      error: {
        code: getObj(errorObj, "code"),
        message:
   ...
   * @param {[{name: string, category: string, type: string, notnull?: boolean, unique?: boolean, primarykey?: boolean, cond?: string }]} columnsData Table columns: database structured
   */
          ...
          colArray.obj.forEach((v) => {
            if (_andor_vals.indexOf(v) === -1) {
              if (
                ...
                let from = new Date(
                  isNaN(_datestr[0]) ? _datestr[0] : Number(_datestr[0])
                );
                from = from.isValid()
                  ? from
                ...
                let to =
                  _datestr.length > 1
                    ? new Date(
                        isNaN(_datestr[1]) ? _datestr[1] : Number(_datestr[1])
                      )
                    : new Date(new Date(from).setHours(24, 0, 0, 0));
                to = to.isValid() ? to : new Date();
                if (to.getTime() === from.getTime())
                  to = new Date(new Date(from).setHours(24, 0, 0, 0));
                _andor_cond.push(
                  `${col} BETWEEN $${
                    query_vals.length + _andor_vals.length + 1
                  } AND $${query_vals.length + _andor_vals.length + 2}`
                );
