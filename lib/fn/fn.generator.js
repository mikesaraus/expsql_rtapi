...
module.exports = {
   ...
   *
   * @param {Boolean} long_string_version - If long version string id; else will return sql syntax
   */
  generateTransactionID: (long_string_version) => {
    const today = new Date()
    return long_string_version === true
      ? [
          String(today.getFullYear()).substring(2) + String(today.getDOY()).padStart(3, 0),
          [
            String(process.env.SRV_UID || 0).padStart(3, 0),
            today.getSeconds(),
      ...
      : [
          `RPAD(concat('`,
          String(today.getFullYear() + today.getDOY())
            .substring(0, 4)
            .padEnd(4, 0),
          String(today.getMilliseconds()).substring(0, 1).padStart(1, 0),
          String(isNaN(process.env.SRV_UID) ? 0 : process.env.SRV_UID)
            .substring(0, 2)
            .padStart(2, 0),
          `'::varchar,`,
