...
module.exports = {
   ...
   *
   * @param {{branch: String?, long_version: Boolean?}} options  Options for generating
   * @param {callback} callback                                  Callback function
   */
  generateTransactionID: (options = { branch: '', long_version: false }, callback) => {
    const { service_view_branches } = require('../../api/users/user.service')
    const today = new Date()
    service_view_branches({}, (err, resp) => {
      const getObj = require('lodash.get')
      const branches = getObj(resp, 'rows', [])
        .filter((d) => d && d.branch_location)
        .map((d) => d.branch_location)
      let branch_id =
        !err && branches.length && branches.indexOf(options.branch) !== -1 ? branches.indexOf(options.branch) + 1 : 0
      return callback(
        options.long_version === true
          ? [
          ...
          : [
              ...
              String(branch_id || 0)
                ...
                .padStart(2, 0),
              `::varchar,`,
              `LPAD((select count(id) from ${process.env.DBTBL_TRANSACTIONS} where date_paid > DATE(CURRENT_TIMESTAMP))::varchar, 3, '0')`,
              `)::varchar, 10, '0')`,
            ].join('')
      )
    })
